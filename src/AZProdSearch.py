...
SearchIndex=Books
Keywords=harry+potter
Timestamp=2016-01-28T08:05:25.000Z
AWSAccessKeyId=AKIAJ4CJCEX5VUXSOEPQ
...
SearchIndex=Books
Service=AWSECommerceService
Timestamp=2016-01-28T08%3A05%3A25.000Z
Version=2011-08-01 2013-08-01

string-to-sign
GET
ecs.amazonaws.com
/onca/xml
AWSAccessKeyId=AKIAJ4CJCEX5VUXSOEPQ&AssociateTag=powersettechc-20&Keywords=harry%20potter&Operation=ItemSearch&SearchIndex=Books&Service=AWSECommerceService&Timestamp=2016-01-28T08%3A05%3A25.000Z&Version=2011-08-01

signed string
http://ecs.amazonaws.com/onca/xml?AWSAccessKeyId=AKIAJ4CJCEX5VUXSOEPQ&AssociateTag=powersettechc-20&Keywords=harry%20potter&Operation=ItemSearch&SearchIndex=Books&Service=AWSECommerceService&Timestamp=2016-01-28T08%3A05%3A25.000Z&Version=2011-08-01&Signature=27q0IUB%2FxOo7carzHl6jzjwoYHxbPn0qFW%2BB2wR6vFk%3D

amazon
http://webservices.amazon.com/onca/xml? Service=AWSECommerceService& AWSAccessKeyId=[AWS Access Key ID]& AssociateTag=[Associate ID]& &Operation=ItemSearch& SearchIndex=Books& Author=Steve%20Davenport& Version=2013-08-01 &Timestamp=[YYYY-MM-DDThh:mm:ssZ] &Signature=[Request Signature]

http://webservices.amazon.com/onca/xml?Service=AWSECommerceService&Operation=ItemSearch&SubscriptionId=AKIAJ4CJCEX5VUXSOEPQ&AssociateTag=powersettechc-20&SearchIndex=Toys&ResponseGroup=Offers&Sort=price&Title=Marvel+Avengers+Age+of+Ultron+Captain+America+Star+Launch+Shield

"""

class AZ(object):
    VERSION = '2013-08-01'  #: supported Amazon API version
    REQUESTS_PER_SECOND = 1  #: max requests per second
    TIMEOUT = 5  #: timeout in seconds
    locale = "us"

    def __init__(self, processor='amazonproduct.processors.objectify'):
    ...
    # def __repr__(self):
    #     return '<API(%s/%s) at %s>' % (
    #         self.VERSION, self.locale, hex(id(self)))
    ...
    def _fetch(self, url):
        ...
        response = request.get(url, stream=True, headers={
    ...
    def item_lookup(self, *ids, **params):
        ...
        * The following requests an iframe that contains customer reviews for
        ...
        try:
            ...
            print("I'm here in the item_lookup")
